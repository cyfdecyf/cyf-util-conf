...
#define SGLIB_HASHED_CONTAINER_FORVAR(type, iter, item) \
    struct sglib_hashed_##type##_iterator iter; \
    type *item

#define SGLIB_HASHED_CONTAINER_FORLOOP(type, iter, item, htab) \
    for (item = sglib_hashed_##type##_it_init(&iter, htab); \
            item != NULL; \
            item = sglib_hashed_##type##_it_next(&iter)) \

#define SGLIB_HASHED_CONTAINER_MAP_ON_ELEMENT(type, item, htab, command) {\
    struct sglib_hashed_##type##_iterator _it_;\
    type *item;\
    for (item = sglib_hashed_##type##_it_init(&_it_, htab);\
            item != NULL;\
            item = sglib_hashed_##type##_it_next(&_it_)) {\
        {command;};\
    }\
}
